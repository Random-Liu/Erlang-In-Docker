...
%%
%% @doc eid_tcp_dist - Erlang in Docker Tcp Connection Protocol
%% By setting 'erl -proto_dist eid_tcp', the net_kernel will use eid_tcp_dist.erl
%% instead of inet_tcp_dist.erl to handle the connection setup between nodes.
%% 
%% In net_kernel.erl(https://github.com/erlang/otp/blob/maint/lib/kernel/src/net_kernel.erl)
%% we can see that net_kernel get connection protocol from an init argument - 'proto_dist'.
%% Then all the listen, accept, setup etc. process are handled in the "protocol module".
%%
%% In eid_tcp_dist.erl, we
%%	1. Fix tcp listen port to ?ERLANG_PORT().
%%	2. Replace epmd port search with dpmd port search.
%% With these, we can build up connection between Erlang VMs in Docker.
...
listen(Name) ->
	...
	{ok, Socket} ->
		%% Causion: The IP in TcpAddress is the container IP.
		TcpAddress = get_tcp_address(Socket),
		{_, Port} = TcpAddress#net_address.address,
		%% We still register the VM on epmd because we need Creation number.
...
do_listen(Options0) ->
	...
	do_listen(ListenPort, [{backlog, 128} | Options]).

do_listen(ListenPort, Options) ->
	case inet_tcp:listen(ListenPort, Options) of
		{ok, Socket} -> {ok, Socket};
		Error -> Error
	end.
...
do_setup(Kernel, Node, Type, MyNode, LongOrShortNames, SetupTime) ->
    ?trace("~p~n",[{eid_tcp_dist, self(), setup, Node}]),
    [Name, Address] = splitnode(Node, LongOrShortNames),
    case inet:getaddr(Address, inet) of
	{ok, Ip} ->
	    Timer = dist_util:start_timer(SetupTime),
		case dpmd:node_to_port(Node) of
		{ok, TcpPort} ->
