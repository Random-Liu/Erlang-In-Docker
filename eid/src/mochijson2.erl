...
%% @doc Added by taotaotheripper
%% A simple function to traverse json term
%% Reference: http://stackoverflow.com/questions/2739384/decode-json-with-mochijson2-in-erlang
-export([jsonobj/1]).
...
%% @spec jsonobj(Struct::json_term()		%% Structure
%%	| {contains, Key::binary()}			%% Check whether item with Key exsits
%%	| {raw, Key::binary()}				%% Get raw data of Key
%%	| Length::len						%% Get Length of list
%%	| Key::binary()						%% Get traverse function of Key
%%	| Index::integer())					%% Get Index'th element
%%	-> term().
jsonobj({struct, List}) ->
	fun({contains, Key}) ->
